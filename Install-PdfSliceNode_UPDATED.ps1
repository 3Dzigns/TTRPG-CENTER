...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
  throw "Docker CLI not found in PATH. Please install Docker Desktop or Docker Engine."
}
...
try {
  ...
  if ($state -ne "true") {
    Write-Warning "Container '$ContainerName' not running. The copy step will fail. Start it or use -PrintComposeSnippet."
  }
} catch {}
...
# IMPORTANT: Fix for PowerShell var-with-colon parsing
